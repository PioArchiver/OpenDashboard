...
(function _Pseudos_() {
    ...
    xtag.pseudos.transition = { 
        onCompiled: function (fn, pseudo) {
            let when = pseudo.arguments[0] || 'immediate',
        ...
        action: function TransitionAction(e, n) {
            xtag.transition(n, e.value, e);
        } 
    };
    ...
    class Model { 
        constructor(_links, readyCallback, readyModelCallback, apploaded) {
            this.requests = { name: "requests", _progress: 0, _loaded: 0, length: 0, keys: [], responses: {} };
            let hrefs = this.getLinkHrefs(_links);
            this.ready = readyCallback; 
            this.openModelLinks(hrefs, readyCallback, readyModelCallback, apploaded);
        ...
        getLinkHrefs(links) {
            ...
            for (var i = 0; i < links.length; i++) {
                ...
                if (link.hasAttribute("data-model") === true) {
                    _urlarray.push(link.getAttribute("href"));
                }
                if (link.hasAttribute("append-resource") === true) {
        ...
        openModelLinks(urls, ready, modelready, _ready) {
            _setprog += 1; 
            if (_setprog === urls.length) { return true; } 
            for (var i = 0; i < urls.length; i++) { 
                var _reqs = this.requests, 
                    _id = urls[i].match(/\w+(?=\.\w+$)/g)[0]; 
                _reqs.length += 1; 
                _reqs.keys.push(_id); 
                _reqs[_id] = this.xhr();
                // Load 
                _reqs[_id].onload = function _XHRLOAD_(e) {
                    let lval = e.target.responseURL.match(/\w+(?=\.\w+$)/g)[0];

                    _reqs.responses[lval] = e.target.response;
                    if (Dashing.LocalDB.model.requests.progress === 1) {
                        modelready(_reqs.responses, true);
                        Dashing.onPioSuccess(e);
                        Dashing.LocalDB.model.requests.progress = 0;
                        Dashing.LocalDB.model.requests._progress = 0;
                    }
                    if (_ready === true && Dashing.LocalDB.model.requests.progress === 1) {
                        Dashing.LocalDB.model.ready(e.target.response);
                        Dashing.onPioSuccess(_reqs);
                    }
                }; 
                // Progress 
                _reqs[_id].onprogress = function _XHRPROGRESSLOAD_(e) {
                    Dashing.LocalDB.model.requests._progress += 1;
                    let _comprat = Dashing.LocalDB.model.requests._progress / Dashing.LocalDB.model.requests.keys.length;
                    Dashing.LocalDB.model.requests.progress = _comprat;
                };
                // Error 
                _reqs[_id].onerror = function _XHRERRORLOAD(e) {
                    Dashing.writer.draw({ name: "_blank", target: Dashing.startscreen }, {
                        parent: {
                            fragment: xtag.createFragment(`<x-message class="error xhr-error"></x-message>`)
                        },
                        child: {
                            omsg: ``
                        }
                    }, undefined, {
                            creator: function Creator(node) {
                                window.setTimeout(function xhrErrorMsgTO() { }, 750);
                             }
                        }); 
                }; 

                // Open and send the request
                _reqs[_id].open("GET", urls[i]); 
                _reqs[_id].send(); 
            } 
        } 
        set ready(def) { Dashing.ModelReady.Ready = def; } 
        get ready() { return Dashing.ModelReady.Ready; } 
    } 
    ...
    class dashboard { 
        constructor(dashed) {
            ...
            ddb.onupgradeneeded = function DashedDBUpgrage(e) {
                ...
                Dashing.LocalDB.success = false;
                Dashing.LocalDB.model = new Dashing.model(importLinks, dashed.onReadyModel, dashed.onModelLoaded || noop);
            ...
            ddb.onsuccess = function DashedDB(e) { 
                ...
                Dashing.LocalDB.upgrade === undefined ? false : Dashing.LocalDB.upgrade; 
                if (!Dashing.LocalDB.upgrade) { Dashing.LocalDB.model = new Dashing.model(importLinks, dashed.onReadyModel, dashed.onModelLoaded || noop, false); } 
                else { Dashing.LocalDB.model = new Dashing.model(importLinks, noop, dashed.onModelLoaded || noop); } 

                Dashing.LocalDB.pioDashed = {
                    version: Dashing.LocalDB.dbconnection.result.version
                };
    ...
    Dashing = new dashboard({
        namespaces: ["x-extension", "x-book", "x-page", "x-table", "x-header", "x-footer", "x-shiftbox", "x-tabbox", "x-modal", "x-message"],
        onStart: function StartShopCenter() {
            ...
            Dashing.LocalDB === undefined ? Dashing.LocalDB = {} : true;
        ...
        onModelLoaded: function OnModelLoaded(doc, test) {
            ...
            console.log(Dashing.LocalDB.model.requests.responses);
            Dashing.rootElement.scheme = doc.JsonSchema; 
            Dashing.rootElement.icos = doc.icos; 

            

        },
        onReadyModel: function OnReadyModel(doc) {
            var jsn = null, frag = null;
            try { /* Write an error message. */ }
            catch (e) {/* Write an error message. */ }
            finally {
        ...
        onPioSuccess: function (requests) {
            // 
        },
        components: function Components(elems) {
            ...
            elems.xPage = class xPage extends HTMLElement {
                ...
                'hide:_transition(boolean,hide,selected,active)'(fn) {
                ...
                'show:_transition(boolean,show,selected,active)'(fn) {
                    if (fn === undefined || fn === false) {
                        return {
                            height: "1s",
                            width: "1s",
                ...
                add(nm, jsn) {
                    if (nm === "plugin") {
                        let skeys = jsn.scripts.keys;
                        for (let k = 0; k < skeys.length; k++) { Dashing.rootElement.addPlugin(skeys[k]); }
                    }
                }
                ...
                attrs() {
                    return {
                        ...
                        activeContent: {
                            get: function GetActivePluginContent() { return this.getAttribute("active-content"); },
                            set: function SetActivePluginContent(val) {
                                ...
                                this.currentContent = Dashing.model.XHR(val, {
                                    ...
                                    onload: function Load(e) {
                                        ...
                                        document.querySelector("#silo-wspace").appendChild(_content);

                                        this.node.add(_jsn, _content, _css);

                                        _script = document.createElement("script");
                                        _script.src = _jsn.scripts.basicSilo;

                                        document.querySelector("#silo-wspace").appendChild(_script);
                                        document.querySelector("#silo-wspace").appendChild(_css[0]);
            ...
            methods() {
                return {
                    ...
                    plugin: function DashioPluginsType(e) {
                        // Has to await the model request
                        if (!Dashing.rootElement.JsonSchema) {
                            window.setTimeout(function pt() { xtag.fireEvent(this, "plugin", { detail: {} }); }, 1000);
                            return false;
                        }
                        console.log(this);
                        let _name = this.getAttribute("plugin-title");
                        if (this.hasAttribute("plugin-title") === false) { return false; }
                        let dbplugin = Dashing.pioDB.prototype.upgrade.apply(this, [_name, Dashing.rootElement.JsonSchema[_name], {
                            transactions: {
