...
(function Dashio() {
    ...
    class pioDB extends Conditions {
        ...
        static open(_name, _version) {
            let _pio = null;
            _version ? _pio = window.indexedDB.open(_name, _version) : _pio = window.indexedDB.open(_name);
            return _pio;
        }
        static upgrade(db, version, options) {
            if (typeof db === "string") {
            ...
            switch (opts.type) {
                ...
                case "templates":
                    for (let i = 0; i < opts.keys.length; i++) {
                        if (Dashing) {
                            Dashing.writer.templater(opts.keys[i], opts[opts.keys[i]]);
                        }
                        else {
    ...
    Dashing = new dashboard({
        ...
        components: function Components(elems) {
            ...
            elems.xPage = class xPage extends HTMLElement {
                ...
                '_hide:transition(boolean,hide,selected,active)'(fn) {
                ...
                '_show:transition(boolean,show,selected,active)'(fn) {
                    if (fn === undefined || fn === false) {
                        return {
                            height: "1s",
                            width: "1s",
            ...
            elems.xModal = class xModal extends HTMLElement {
                ...
                'show:transition(boolean,show,focus,hidden)'(fn) {
                    if (fn === undefined || fn === false) {
                        return {
                            height: "2s",
                            width: "2s",
                ...
                'hide:transition(boolean,hide,focus,hidden)'(fn) {
                    if (fn === undefined || fn === false) {
                        return {
                            height: "1s",
                            width: "1s",
